...
def stats(update, context):
    ...
    if config_dict['EMOJI_THEME']:
            stats = f'<b>       馃搳 Bot Statistics </b>\n' \
                    f'馃洜 Updated On: {last_commit}\n'\
                    f'鈱� Uptime: {currentTime}\n'\
                    f'馃 Version: {version}\n'\
                    f'馃煝 OS Uptime: {osUptime}\n'\
                    f'馃枼锔� CPU: [{progress_bar(cpuUsage)}] {cpuUsage}%\n'\
                    f'馃幃 RAM: [{progress_bar(mem_p)}] {mem_p}%\n'\
                    f'馃捑 Disk: [{progress_bar(disk)}] {disk}%\n'\
                    f'馃捒 Disk Free: {free}\n'\
                    f'馃敽 Upload Data: {sent}\n'\
                    f'馃敾 Download Data: {recv}\n\n'
    ...
    if config_dict['SHOW_LIMITS_IN_STATS']:
        ...
        if config_dict['EMOJI_THEME']: 
            stats += f'<b>馃敘 Bot Limitations </b>\n'\
                     f'馃Р Torrent/Direct: {torrent_direct}\n'\
                     f'馃攼 Zip/Unzip: {zip_unzip}\n'\
                     f'馃敺 Leech: {leech_limit}\n'\
                     f'鈾伙笍 Clone: {clone_limit}\n'\
                     f'馃敯 Mega: {mega_limit}\n'\
                     f'馃挘 Total Tasks: {total_task}\n'\
                     f'馃敨 User Tasks: {user_task}\n\n'
        else: 
            stats += f'<b>馃敘 Bot Limitations </b>\n'\
...
def ping(update, context):
    if config_dict['EMOJI_THEME']:
        ...
        reply = sendMessage("Starting_Ping 鉀�", context.bot, update.message)
        end_time = int(round(time() * 1000))
        editMessage(f'{end_time - start_time} ms 馃敟', reply)
    else:
        start_time = int(round(time() * 1000))
...
<b><u>馃懁 User Commands</u></b>
<br><br>
鈥� <b>/{BotCommands.HelpCommand}</b>: To get this message
<br><br>
鈥� <b>/{BotCommands.MirrorCommand[0]}</b> [download_url][magnet_link]: Start mirroring to Google Drive. Send <b>/{BotCommands.MirrorCommand[0]}</b> for more help
<br><br>
鈥� <b>/{BotCommands.ZipMirrorCommand[0]}</b> [download_url][magnet_link]: Start mirroring and upload the file/folder compressed with zip extension
<br><br>
鈥� <b>/{BotCommands.UnzipMirrorCommand[0]}</b> [download_url][magnet_link]: Start mirroring and upload the file/folder extracted from any archive extension
<br><br>
鈥� <b>/{BotCommands.QbMirrorCommand[0]}</b> [magnet_link][torrent_file][torrent_file_url]: Start Mirroring using qBittorrent, Use <b>/{BotCommands.QbMirrorCommand[0]} s</b> to select files before downloading
<br><br>
鈥� <b>/{BotCommands.QbZipMirrorCommand[0]}</b> [magnet_link][torrent_file][torrent_file_url]: Start mirroring using qBittorrent and upload the file/folder compressed with zip extension
<br><br>
鈥� <b>/{BotCommands.QbUnzipMirrorCommand[0]}</b> [magnet_link][torrent_file][torrent_file_url]: Start mirroring using qBittorrent and upload the file/folder extracted from any archive extension
<br><br>
鈥� <b>/{BotCommands.LeechCommand[0]}</b> [download_url][magnet_link]: Start leeching to Telegram, Use <b>/{BotCommands.LeechCommand[0]} s</b> to select files before leeching
<br><br>
鈥� <b>/{BotCommands.ZipLeechCommand[0]}</b> [download_url][magnet_link]: Start leeching to Telegram and upload the file/folder compressed with zip extension
<br><br>
鈥� <b>/{BotCommands.UnzipLeechCommand[0]}</b> [download_url][magnet_link][torent_file]: Start leeching to Telegram and upload the file/folder extracted from any archive extension
<br><br>
鈥� <b>/{BotCommands.QbLeechCommand[0]}</b> [magnet_link][torrent_file][torrent_file_url]: Start leeching to Telegram using qBittorrent, Use <b>/{BotCommands.QbLeechCommand[0]} s</b> to select files before leeching
<br><br>
鈥� <b>/{BotCommands.QbZipLeechCommand[0]}</b> [magnet_link][torrent_file][torrent_file_url]: Start leeching to Telegram using qBittorrent and upload the file/folder compressed with zip extension
<br><br>
鈥� <b>/{BotCommands.QbUnzipLeechCommand[0]}</b> [magnet_link][torrent_file][torrent_file_url]: Start leeching to Telegram using qBittorrent and upload the file/folder extracted from any archive extension
<br><br>
鈥� <b>/{BotCommands.CloneCommand[0]}</b> [drive_url][gdtot_url]: Copy file/folder to Google Drive
<br><br>
鈥� <b>/{BotCommands.CountCommand}</b> [drive_url][gdtot_url]: Count file/folder of Google Drive
<br><br>
鈥� <b>/{BotCommands.DeleteCommand}</b> [drive_url]: Delete file/folder from Google Drive (Only Owner & Sudo)
<br><br>
鈥� <b>/{BotCommands.YtdlCommand[0]}</b> [yt-dlp supported link]: Mirror yt-dlp supported link. Send <b>/{BotCommands.YtdlCommand[0]}</b> for more help
<br><br>
鈥� <b>/{BotCommands.YtdlZipCommand[0]}</b> [yt-dlp supported link]: Mirror yt-dlp supported link as zip
<br><br>
鈥� <b>/{BotCommands.YtdlLeechCommand[0]}</b> [yt-dlp supported link]: Leech yt-dlp supported link
<br><br>
鈥� <b>/{BotCommands.YtdlZipLeechCommand[0]}</b> [yt-dlp supported link]: Leech yt-dlp supported link as zip
<br><br>
鈥� <b>/{BotCommands.UserSetCommand[0]}</b>: Users settings
<br><br>
鈥� <b>/{BotCommands.RssListCommand}</b>: List all subscribed rss feed info
<br><br>
鈥� <b>/{BotCommands.RssGetCommand}</b>: [Title] [Number](last N links): Force fetch last N links
<br><br>
鈥� <b>/{BotCommands.RssSubCommand}</b>: [Title] [Rss Link] f: [filter]: Subscribe new rss feed
<br><br>
鈥� <b>/{BotCommands.RssUnSubCommand}</b>: [Title]: Unubscribe rss feed by title
<br><br>
鈥� <b>/{BotCommands.RssSettingsCommand}</b>: Rss Settings
<br><br>
鈥� <b>/{BotCommands.CancelMirror}</b>: Reply to the message by which the download was initiated and that download will be cancelled
<br><br>
鈥� <b>/{BotCommands.CancelAllCommand}</b>: Cancel all downloading tasks
<br><br>
鈥� <b>/{BotCommands.ListCommand}</b> [query]: Search in Google Drive(s)
<br><br>
鈥� <b>/{BotCommands.SearchCommand}</b> [query]: Search for torrents with API
<br>sites: <code>rarbg, 1337x, yts, etzv, tgx, torlock, piratebay, nyaasi, ettv</code><br><br>
鈥� <b>/{BotCommands.StatusCommand}</b>: Shows a status of all the downloads
<br><br>
鈥� <b>/{BotCommands.StatsCommand}</b>: Show Stats of the machine the bot is hosted on
<br><br>
鈥� <b>/{BotCommands.SpeedCommand[0]}</b>: Speedtest of server
<br><br>
鈥� <b>/weebhelp</b>: Okatu helper
'''
...
<b><u>馃洝锔� Admin Commands</u></b>
<br><br>
鈥� <b>/{BotCommands.PingCommand}</b>: Check how long it takes to Ping the Bot
<br><br>
鈥� <b>/{BotCommands.AuthorizeCommand}</b>: Authorize a chat or a user to use the bot (Can only be invoked by Owner & Sudo of the bot)
<br><br>
鈥� <b>/{BotCommands.UnAuthorizeCommand}</b>: Unauthorize a chat or a user to use the bot (Can only be invoked by Owner & Sudo of the bot)
<br><br>
鈥� <b>/{BotCommands.UsersCommand}</b>: show users settings (Only Owner & Sudo).
<br><br>
鈥� <b>/{BotCommands.AddSudoCommand}</b>: Add sudo user (Only Owner)
<br><br>
鈥� <b>/{BotCommands.RmSudoCommand}</b>: Remove sudo users (Only Owner)
<br><br>
鈥� <b>/{BotCommands.PaidUsersCommand}</b>: Show Paid users (Only Owner & Sudo)
<br><br>
鈥� <b>/{BotCommands.AddPaidCommand}</b>: Authorize Paid users (Only Owner)
<br><br>
鈥� <b>/{BotCommands.RmPaidCommand}</b>: Unauthorize Paid users (Only Owner)
<br><br>
鈥� <b>/{BotCommands.RestartCommand}</b>: Restart and update the bot (Only Owner & Sudo)
<br><br>
鈥� <b>/{BotCommands.LogCommand}</b>: Get a log file of the bot. Handy for getting crash reports
'''
...
def bot_help(update, context):
    button = ButtonMaker()
    if config_dict['EMOJI_THEME']:
        button.buildbutton("馃懁 User", f"https://telegra.ph/{help_user}")
        button.buildbutton("馃洝锔� Admin", f"https://telegra.ph/{help_admin}")
    else:
...
def main():
        ...
        if notifier_dict := DbManger().get_incomplete_tasks():
            for cid, data in notifier_dict.items():
                if ospath.isfile(".restartmsg"):
                    ...
                    msg = f"Bot Restarted Successfully鉂梊n"
                else:
                    msg = f"Bot Restarted!\n"
                msg += f"Date: {date}\n"
                msg += f"Time: {time}\n"
                ...
                for tag, links in data.items():
                    ...
                    for index, link in enumerate(links, start=1):
                        ...
                        if len(msg.encode()) > 4000:
                            if 'Bot Restarted Successfully鉂�' in msg and cid == chat_id:
                                try:
    ...
    LOGGER.info("馃挜 饾悂饾惃饾惌 饾悞饾惌饾悮饾惈饾惌饾悶饾悵鉂�")
