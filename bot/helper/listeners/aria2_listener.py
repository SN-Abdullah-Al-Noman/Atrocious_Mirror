...
async def _onDownloadStarted(api, gid):
    ...
    if task := await getTaskByGid(gid):
        ...
        if msg:
            await task.listener.onDownloadError(msg)
        ...
        msg = await stop_duplicate_leech(name=task.listener.name, size=download.total_length, listener=task.listener)
        if msg:
            await task.listener.onDownloadError(msg, button)
            await sync_to_async(api.remove, [download], force=True, files=True)
            return
        ...
        if config_dict['MIRROR_LIMIT'] or config_dict['TORRENT_LIMIT'] or config_dict['LEECH_LIMIT'] or config_dict['STORAGE_THRESHOLD']:
            ...
            if download.is_torrent:
                limit_exceeded, button = await limit_checker(size=download.total_length, listener=task.listener, isTorrent=True)
                if limit_exceeded:
                    await task.listener.onDownloadError(limit_exceeded, button)
                    await sync_to_async(api.remove, [download], force=True, files=True)
            else:
                limit_exceeded, button = await limit_checker(size=download.total_length, listener=task.listener)
                if limit_exceeded:
                    await task.listener.onDownloadError(limit_exceeded, button)
                    await sync_to_async(api.remove, [download], force=True, files=True)
