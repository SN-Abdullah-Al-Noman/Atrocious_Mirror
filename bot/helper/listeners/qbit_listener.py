...
from bot.modules.func import check_filename, limit_checker, stop_duplicate_leech
...
@new_task
async def _check_filename(tor):
    task = await getTaskByGid(tor.hash[:12])
    if not hasattr(task, "listener"):
        return
    task.listener.name = tor.content_path.rsplit("/", 1)[-1].rsplit(".!qB", 1)[0]
    msg = await check_filename(message=task.listener.message, file_name=task.listener.name, link=task.listener.link)
    if msg:
        _onDownloadError(msg, tor)
...
async def _size_checked(tor):
    ...
    if not hasattr(task, "listener"):
        return
    size = tor.size
    limit_exceeded, button = await limit_checker(size, listener=task.listener, isTorrent=True)
    if limit_exceeded:
        await _onDownloadError(limit_exceeded, tor, button)


@new_task
async def _stop_duplicate_leech(tor):
    task = await getTaskByGid(tor.hash[:12])
    if not hasattr(task, "listener"):
        return
    task.listener.name = tor.content_path.rsplit("/", 1)[-1].rsplit(".!qB", 1)[0]
    msg = await stop_duplicate_leech(task.listener.name, task.listener.size, task.listener)
    if msg:
        _onDownloadError(msg, tor)
        return
            ...
            try:
                ...
                for tor_info in torrents:
                    ...
                    if state == "metaDL":
                        ...
                        if (config_dict["STOP_DUPLICATE_LEECH"] and not QbTorrents[tag]["stop_dup_leech_check"]):
                            QbTorrents[tag]["stop_dup_leech_check"] = True
                            _stop_duplicate_leech(tor_info)
                        ...
                        if (config_dict["BLACKLIST_FILE_KEYWORDS"] and not QbTorrents[tag]["check_filename"]):
                            QbTorrents[tag]["check_filename"] = True
                            _check_filename(tor_info)
