...
async def add_gd_download(listener, path):
    ...
    msg = await check_filename(message=listener.message, file_name=listener.name, link=listener.link)
    if msg:
    ...
    msg = await stop_duplicate_leech(listener.name, listener.size, listener)
    if msg:
        await listener.onDownloadError(msg)
        return

    limit_exceeded, button = await limit_checker(listener.size, listener=listener, isDriveLink=True)
    if limit_exceeded:
